...
def create_task(task: TaskCreate, background_tasks: BackgroundTasks, db: Session = Depends(get_db),             
  current_user: User = Depends(get_current_user), _: None = Depends(user_rate_limiter)  
):
  db_task = crud.create_task(db=db, title=task.title, description=task.description, owner_id=current_user.id)
  background_tasks.add_task(notify_task_created, email=current_user.email, task_title=db_task.title, user_id=current_user.id)
