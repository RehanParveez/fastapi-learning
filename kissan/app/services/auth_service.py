...
def register_user(db: Session, user_in: UserCreate) -> User:
  ...
  if user_in.username:
    ...
    try:
      return user_repository.create_user(db, phone=user_in.phone, email=user_in.email,
      username=user_in.username, gender=user_in.gender, hashed_password=hashed, role=user_in.role)
    except IntegrityError:
      db.rollback()
      raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="A user with this phone or email already exists")
