...
def create_user(
  db: Session, *, phone: str, email: str | None, username: str | None = None,
  gender: Gender | None = None, hashed_password: str, role: UserRole
) -> User:
  db_user = User(phone=phone, email=email, username=username, gender=gender, hashed_password=hashed_password, role=role)
  db.add(db_user)
  db.commit()
  db.refresh(db_user)
  return db_user
